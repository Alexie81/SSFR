...
**Rutare spectrală certificată pentru căutare vectorială distribuită la scară mare**

`Research Prototype` · `Vector Search` · `Fourier Routing` · `Deterministic Bounds` · `Python`

> [!IMPORTANT]
> **Statutul proiectului:** SSFR este un prototip experimental. Catalogul de **1.000.000 de produse** a fost generat și validat fizic, însă este sintetic. Configurația pentru **1 miliard de vectori** reprezintă o proiecție de capacitate, nu un benchmark executat fizic la acea scară.

> [!TIP]
> Formulele folosesc sintaxa matematică Markdown compatibilă cu GitHub: `$...$` pentru formule inline și blocuri `math` pentru ecuațiile afișate separat.

## Rezumat executiv

SSFR reduce costul rutării unei interogări vectoriale prin comprimarea spectrală a matricei de centroizi pe axa shardurilor. Routerul estimează scorurile shardurilor în domeniul Fourier, construiește intervale deterministe de eroare și extinde adaptiv banda spectrală până când selecția poate fi certificată. Dacă certificarea nu este posibilă, sistemul revine conservator la rutarea exactă.

| Caracteristică | Implementare SSFR |
|---|---|
| **Scop** | selectarea rapidă a shardurilor relevante înaintea căutării locale |
| **Mecanism central** | ordonare de centroizi + rFFT + scoruri aproximative |
| **Controlul erorii** | reziduuri de reconstrucție și intervale deterministe |
| **Corectitudine** | certificare top-$P$, certificare separată pentru pruning și fallback exact |
| **Integrare** | CLI, FastAPI, filtre e-commerce, Exact, HNSW și FAISS |
| **Scalare demonstrată** | catalog fizic sintetic cu 1.000.000 de produse |

## Cuprins

1. [Prezentarea proiectului](#prezentarea-proiectului)
2. [Contribuția tehnică](#contribuția-tehnică)
3. [Rezultate validate și estimări](#rezultate-validate-și-estimări)
4. [Arhitectura sistemului](#arhitectura-sistemului)
5. [Pornire rapidă](#pornire-rapidă)
6. [Catalogul fizic de 1000000 de produse](#catalogul-fizic-de-1000000-de-produse)
7. [Notație și ipoteze matematice](#notație-și-ipoteze-matematice)
8. [Modelul matematic complet](#modelul-matematic-complet)
9. [Complexitate temporală](#complexitate-temporală)
10. [Complexitate spațială](#complexitate-spațială-și-formule-de-memorie)
11. [Proiecția pentru 1 miliard](#proiecție-de-capacitate-pentru-1-miliard-de-vectori)
12. [API, teste și limitări](#api-fastapi)
...
Fluxul SSFR separă explicit **rutarea globală** de **căutarea locală**:

1. construiește sharduri și centroizi normalizați;
2. ordonează centroizii astfel încât vecinii semantici să fie apropiați pe axa shardurilor;
3. comprimă această secvență prin rFFT;
4. proiectează interogarea direct în domeniul frecvență;
5. estimează scorurile tuturor shardurilor;
6. calculează limite inferioare și superioare pentru fiecare scor;
7. certifică selecția top-$P$ sau mărește banda spectrală;
8. folosește fallback exact atunci când dovada nu poate fi obținută;
9. caută în paralel în shardurile selectate și efectuează merge-ul global top-$k$.

> [!NOTE]
> Certificarea selecției centroizilor și certificarea pruningului la nivelul vectorilor sunt două garanții distincte. Un recall empiric ridicat nu înlocuiește demonstrația matematică.

---

## Rezultate validate și estimări
...
| End-to-end pentru 1 miliard | **12,971 ms** | ⚠️ estimare bazată pe ipoteze de cluster |
...
## Catalogul fizic de 1.000.000 de produse
...
```math
\bar c_i = \frac{1}{n_i}\sum_{x\in\mathcal{S}_i}x, \qquad c_i = \frac{\bar c_i}{\lVert\bar c_i\rVert_2}
```
...
```math
F_k = \sum_{n=0}^{S-1} \widetilde c_n e^{-2\pi i kn/S}, \qquad k=0,\ldots,S-1
```
...
```math
\widehat c_n^{(b)} = \frac{1}{S} \sum_{k\in\mathcal{K}_b} F_k e^{2\pi i kn/S}
```
...
```math
\widehat s_n^{(b)} = \mathrm{IFFT}(z)_n = q^\top\widehat c_n^{(b)}
```
...
```math
r_i^{(b)} = \left\lVert c_i-\widehat c_i^{(b)} \right\rVert_2
```

Din inegalitatea Cauchy–Schwarz:

```math
\left| q^\top c_i-q^\top\widehat c_i^{(b)} \right| \le \lVert q\rVert_2 r_i^{(b)}
```

Rezultă intervalul determinist asociat fiecărui shard:

**Limită inferioară:**

```math
L_i^{(b)} = \widehat s_i^{(b)}-\lVert q\rVert_2r_i^{(b)}
```

**Limită superioară:**

```math
U_i^{(b)} = \widehat s_i^{(b)}+\lVert q\rVert_2r_i^{(b)}
```
...
```math
\min_{i\in\mathcal{T}} L_i^{(b)} \ge \max_{j\notin\mathcal{T}} U_j^{(b)}
```
...
```math
q^\top x \le U_i^{(b)}+\lVert q\rVert_2\rho_i
```
...
```math
\max_{j\notin\mathcal{T}} \left( U_j^{(b)}+\lVert q\rVert_2\rho_j \right) \le \tau_k
```
...
```math
F'_k = F_k + \Delta c_n e^{-2\pi i kn/S}
```
...
```math
T_{\text{exact-router}} = O(Sd)
```

O încercare spectrală:

```math
T_{\text{spectral}} = O(bd + S\log S + S + P\log P)
```

Pentru $T$ încercări cu prefixuri de frecvență reutilizate:

```math
T_{\text{adaptive}} = O\left( b_{\max}d + TS\log S + TS + TP\log P \right)
```

Căutarea exactă în $P$ sharduri echilibrate:

```math
T_{\text{local-exact}} \approx O\left( \frac{PNd}{S} \right)
```
...
```math
T_{\text{all}} = O(Nd + N\log N), \qquad M_{\text{all}}=O(N)
```
...
## Complexitate spațială și formule de memorie
...
```math
M_{\text{vectors-current}} \approx 8Nd\ \text{bytes}
```
...
```math
M_{F,\text{disk}} = 16d\sum_{\ell=1}^{L}(b_\ell+1)
```
...
```math
M_{\text{IDs}} = 32N\ \text{bytes/copie}
```

Cu o copie globală și una în sharduri:

```math
M_{\text{IDs-current}} \approx 64N\ \text{bytes}
```

Assignments:

```math
M_{\text{assignments}} = 8N\ \text{bytes}
```

---

## Resurse pentru configurația de 1.000.000
...
```math
311{,}403 + 415{,}984 + 557{,}071 \approx 1{,}254\ \text{GiB}
```
...
## Proiecție de capacitate pentru 1 miliard de vectori
...
```math
T_{\text{e2e}} \approx T_{\text{router}} + \left\lceil\frac{P}{W}\right\rceil \left( T_{\text{local,p95}}+T_{\text{network,p95}} \right) + T_{\text{merge}}
```
...
```math
T_{\text{e2e}} \approx 2{,}971+8+1{,}5+0{,}5 = 12{,}971\ \text{ms}
```
...
## Garanții matematice și limite de interpretare
...
**SSFR urmărește să combine rutarea spectrală rapidă cu garanții matematice explicite și un fallback exact conservator.**
